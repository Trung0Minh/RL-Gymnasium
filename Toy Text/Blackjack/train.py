...
def train(cfg: QConfig):
    env = gym.make(cfg.env_id, sab=cfg.sab)
    agent = BlackjackAgent(config=cfg)
    
    checkpoint_path = f"{cfg.checkpoint_dir}/blackjack_q.pkl"
    ...
    rewards = []
    rewards_window = deque(maxlen=100000)
    ...
    for i_episode in range(1, cfg.num_episodes + 1):
        ...
        log_str = f"Episode {i_episode}\tAverage Reward: {avg_reward:.4f}\tEpsilon: {agent.epsilon:.4f}"
...
if __name__ == "__main__":
    ...
    plt.title('DQN Training Rewards for Blackjack')
