...
def test():
    ...
    model = ActorCritic(obs_dim, action_dim, hidden_dim=512).to(device)
    model.load_state_dict(torch.load("checkpoints/humanoid_standup_ppo.pt", map_location=device, weights_only=False))
    model.eval()

    # Load normalization stats
    stats = torch.load("checkpoints/humanoid_standup_obs_rms.pt", map_location=device, weights_only=False)
    mean = stats["mean"]
    var = stats["var"]
