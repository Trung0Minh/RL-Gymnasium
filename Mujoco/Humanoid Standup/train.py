...
def train(cfg: PPOConfig):
    ...
    if cfg.resume:
        model_path = f"{cfg.checkpoint_dir}/humanoid_standup_ppo.pt"
        stats_path = f"{cfg.checkpoint_dir}/humanoid_standup_obs_rms.pt"
    ...
    for update in range(1, cfg.num_updates + 1):
        ...
        if update % 10 == 0:
            ...
            torch.save(agent.network.state_dict(), f"{cfg.checkpoint_dir}/humanoid_standup_ppo.pt")
            torch.save({
                "mean": obs_rms_wrapper.obs_rms.mean,
                "var": obs_rms_wrapper.obs_rms.var
            }, f"{cfg.checkpoint_dir}/humanoid_standup_obs_rms.pt")
