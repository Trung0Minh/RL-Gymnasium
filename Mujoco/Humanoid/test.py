...
def test():
    ...
    model.load_state_dict(torch.load("checkpoints/humanoid_ppo.pt", map_location=device, weights_only=False))
    model.eval()

    stats = torch.load("checkpoints/humanoid_obs_rms.pt", map_location=device, weights_only=False)
    mean = stats["mean"]
    var = stats["var"]
