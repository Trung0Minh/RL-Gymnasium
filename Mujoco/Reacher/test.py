...
def test():
    ...
    model.load_state_dict(torch.load("checkpoints/reacher_ppo.pt", map_location=device, weights_only=False))
    model.eval()

    # Load normalization stats
    stats = torch.load("checkpoints/reacher_obs_rms.pt", map_location=device, weights_only=False)
    mean = stats["mean"]
    var = stats["var"]
