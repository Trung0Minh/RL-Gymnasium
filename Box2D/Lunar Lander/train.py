...
def train(cfg: DQNConfig):
    ...
    # DQN usually works better with a single env unless using a specialized variant
    # but we will use the standard structure for consistency.
    ...
    for i_episode in range(1, cfg.num_episodes + 1):
        ...
        for t in range(cfg.max_t):
            ...
            # Since we are using vectorized envs, but DQN is typically single-env:
            # We'll act for each env. DQNAgent.act is currently for single state.
            # Let's adapt it or just use the first env's state if num_envs=1.
            if cfg.num_envs == 1:
                action = [agent.act(state[0], eps)]
            else:
                # Basic vectorization support for act
                actions = []
                for s in state:
...
if __name__ == "__main__":
    ...
    plt.ylabel('Score')
    plt.xlabel('Episode #')
    plt.title("DQN Training Scores for Lunar Lander")
